...
def main():
    ...
    print("3. テストモード（代表的な地点での区切りf-kフィルタリング）")
    ...
    if mode == '3':
        ...
        print("代表的な地点での f-k フィルタリングを実行します")
    ...
    # モード3: テストモード（代表点での区切りf-kフィルタリング）
    if mode == '3':
        ...
        for result in tqdm(test_results, desc="フィルタリング中"):
            try:
                # フィルター適用
                KK_filtered = apply_fk_filter(result['data'], result['f_MHz'], result['K'], 
                                            filter_type, **filter_params)
                ...
                filtered_test_results.append({
                    ...
                    'point_idx': result['point_idx']
                })
                
            except Exception as e:
                print(f"代表点 {result['point_idx']} でフィルタリングエラー: {e}")
                continue
        
        print("各代表点の結果を保存中...")
        for result in tqdm(filtered_test_results, desc="保存中"):
            ...
            plot_fk_filter_comparison(
                result['original_data'], result['filtered_data'],
                result['KK_original'], result['KK_filtered'],
                result['f_MHz'], result['K'], png_path, 
                f"({filter_type})"
            )
            ...
            plot_fk_filter_comparison(
                result['original_data'], result['filtered_data'],
                result['KK_original'], result['KK_filtered'],
                result['f_MHz'], result['K'], pdf_path, 
                f"({filter_type})"
            )
        
        print(f"テストモード結果を保存: {test_dir}")
        print(f"代表点数: {len(filtered_test_results)}")
